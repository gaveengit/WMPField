...
public class AddOviServiceAdditionalActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.add_ovservice_additional);
    ...
    public void submitOvi(View v) {
        ...
        OviServiceModel oviServiceModel = new OviServiceModel(ovi_run_id, ovi_trap_id, service_id, currentDate, current_time, service_status);
        long flag = dbHandler.updateSingleOviService(oviServiceModel);
        if (flag != -1) {
            Toast.makeText(context, "OVI Service has been updated successfully.",
                    Toast.LENGTH_LONG).show();
            Intent intent = new Intent(context, OvListActivity.class);
            intent.putExtra("type", "ov");
            startActivity(intent);
        } else {
            Toast.makeText(context, "Failure in adding OVI service. Please try again.",
                    Toast.LENGTH_LONG).show();
            Intent intent = new Intent(context, OvListActivity.class);
            intent.putExtra("type", "ov");
            startActivity(intent);
